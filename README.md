...
Se tiver um arquivo backup **.sql** e quiser restaurá-lo, use **docker cp caminho/do/seu/backup.sql nome-container:/backup.sql** para copiá-lo, depois entre no container (**docker exec -it nome-container bash**) e rode **mysql -u root -p nome-bd < /backup.sql**. Ou **docker exec -i nome-container mysql -u root -p12345678 nome_BD < ../dump.sql (caminho dump)**
